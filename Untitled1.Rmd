...
plot.ts(milk.bc,xlab = "", main = "")
hist(milk.bc, col = "light blue", xlab = "", main = "") 
qqnorm(milk.bc, main = "", xlab = "")
qqline(milk.bc, col = "red")
```
```{r}
y <- ts(as.ts(milk.bc), frequency = 12)
decomp <- decompose(y)
plot(decomp)
```
...
milk.bc_12_1_1 <- diff(milk.bc_12_1, lag=1)
var(milk.bc_12_1_1)
```
...
plot.ts(milk.log_12_1, main = "bc(U_t) differenced at lag 12, at lag 1")
...
par(mfrow=c(2,3))
acf(milk.bc, lag.max = 40, main = expression(U[t]))
acf(milk.bc_12, lag.max = 40, main = expression(nabla[12]~~U[t])) 
acf(milk.bc_12_1, lag.max = 40, main =expression(nabla[1]~nabla[12]~~U[t]))
pacf(milk.bc, lag.max = 40, main = "")
pacf(milk.bc_12, lag.max = 40, main = "")
pacf(milk.bc_12_1, lag.max = 40, main = "")
```
```{r}
# Compare the histgram of milk.log, milk.log_12, and milk.log_12_1
hist(milk.bc, density=20,breaks=20, col="blue", xlab="", prob=TRUE)
hist(milk.bc_12, density=20,breaks=20, col="blue", xlab="", prob=TRUE)
...
a <- AICc(arima(milk.bc, order = c(0,1,1), seasonal = list(order = c(0,1,4), period = 12),method = "ML"))
a
```


```{r}
arima(milk.bc, order = c(0,1,1), seasonal = list(order = c(0,1,4), period = 12),method = "ML")
```
...
```{r}
arima(milk.bc, order=c(0,1,1), seasonal = list(order = c(0,1,4), period = 12), fixed = c(NA,NA,0,0,NA), method="ML")
```
```{r}
# Calculate the AICc of possible models
c <- AICc(arima(milk.bc, order=c(0,1,1), seasonal = list(order = c(0,1,4), period = 12), fixed = c(NA,NA,0,0,NA), method="ML"))
c
```
```{r}
arima(milk.bc, order=c(0,1,1), seasonal = list(order = c(0,1,4), period = 12), fixed = c(NA,NA,0,0,NA), method="ML")
...
$\nabla[1]~\nabla[12] bc (Ut) = (1–0.1908B) (1-0.7076B^{12}+0.2888B^{48}) Zt$
```{r}
polyroot(c(1, -0.1908))
```
...
library(UnitCircle)
par(mfrow=c(1,2))
uc.check(pol_ = c(1,-0.7076,0,0,0.2888), plot_output = TRUE,print_output = F) # Seasonal MA Part 
uc.check(pol_ = c(1,-0.1908), plot_output = TRUE,print_output = F) # Non-Seasonal MA Part
```

```{r}
# Plot the residuals
fit1 <- arima(milk.bc, order = c(0,1,1),seasonal = list(order = c(0,1,4), period = 12), method = "ML")
res1 <- residuals(fit1)
plot.ts(res1, xlab = "")
