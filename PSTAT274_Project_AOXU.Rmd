...
### Model Estimation
```{r}
c <- AICc(arima(milk, order = c(1,1,0), seasonal = list(order = c(3,1,4), period = 12),method = "ML"))
c
```
...
shapiro.test(res1)
Box.test(res1,lag = 12, type = c("Box-Pierce"),fitdf = 3) 
Box.test(res1,lag = 12, type = c("Ljung-Box"),fitdf = 3) 
