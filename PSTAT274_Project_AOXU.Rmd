...
It cannot pass the Shapiro_Wilk normality test due to the influence of right tail. Also, there are two leverage points on the right side at qq-norm, which can affect my residual's normality.
...
We are happy to see that all testing set data are within the 95% confidence interval of our prediction model. Also, the values predicted by the selected model are closed to the actual values, which indicate our model is sufficient and proper to forecast milk production.

# **2.0 CONCLUSION**
After comparing AICc values of two possible models - $SARIMA(0,1,1)(0,1,1)_{12}$ and $SARIMA(0,1,1)(0,1,4)_{12}$. The model $SARIMA(0,1,1)(0,1,4)_{12}$ has smaller AICc and provide credible forecasting results for our data set. It passes all diagnostic tests but not Shapiro-Wilk normality test. That's because the originial data is non-gaussian distribution, but the box-cox jenkins is already the best one. Also, it cannot pass the Shapiro_Wilk normality test due to the influence of right tail. There are two levergae opints on the right side at my qq-norm, which can affect my residual's normality. However, we are still confident to claim that our $SARIMA(0,1,1)(0,1,4)_{12}$ model is appropriate to forecast the monthly measurements of milk production (pounds per cow).

The final model is: 
$$\nabla_{1}\nabla_{12}ln(U_t) = (1-0.1729B)(1-0.6771B^{12}+0.3054B^{48})Z_t$$ with $$Z_t - WN(0,\sigma_z^2)$$  and $$\sigma_z^2 = 9.928*10^{-5}$$
\newpage
# **3.0 REFERENCES**
...
y1 <- ts(as.ts(c_train),frequency = 12)
decomp <- decompose(y1)
plot(decomp)
# Remove seasonality and trend
c_train_12 <- diff(c_train, lag = 12)
c_train_12_1 <- diff(c_train_12, lag = 1)
c_train_12_1_1 <- diff(c_train_12_1, lag = 1)
# To check number differencing
par(mfrow=c(1,2))
plot.ts(c_train_12, main = "")
abline(h=mean(c_train_12), col = "blue")
plot.ts(c_train_12_1, main = "")
abline(h=mean(c_train_12_1), col = "blue")
# Create table to compare variance
v <- cbind(var(c_train), var(c_train_12), var(c_train_12_1), var(c_train_12_1_1))
rownames(v) <- "Variance"
colnames(v) <- c("$U_t$","$\\nabla_{12}$$U_t$",
                 "$\\nabla_{1}$$\\nabla_{12}$$U_t$",
                 "$\\nabla_{1}$$\\nabla_{1}$$\\nabla_{12}$$U_t$")
knitr::kable(v)
# Compare the ACF and PACF of c_train, c_train_12, and c_train_12_1
par(mfrow=c(2,3))
acf(c_train, lag.max = 40, main = expression(U[t]))
acf(c_train_12, lag.max = 40, main = expression(nabla[12]~~U[t]))
acf(c_train_12_1, lag.max = 40, main = expression(nabla[1]~nabla[12]~~U[t]))
pacf(c_train, lag.max = 40, main = "")
pacf(c_train_12, lag.max = 40, main = "")
pacf(c_train_12_1, lag.max = 40, main = "")
```
