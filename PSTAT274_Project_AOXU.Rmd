...
c_test <-ts(milk[157:168],start = c(1975,12),frequency = 12)
...
lambda
```
$\lambda$ = 0 (log) is also in the confidence interval and very close to $\lambda$ = 0.02020202.
So we choose log transform.
...
a <- AICc(arima(milk.log, order = c(1,1,1), seasonal = list(order = c(3,1,4), period = 12), method = "CSS-ML"))
a
```

```{r}
arima(milk.log, order=c(1,1,1), seasonal = list(order = c(3,1,4), period = 12), method="CSS")
```
Since ar1, ma1, sma1, sma2, sma3, sma4 include 0, then we choose NA.
...
```{r}
arima(milk.log, order=c(1,1,1), seasonal = list(order = c(3,1,4), period = 12), fixed = c(0,0,NA,NA,NA,0,0,0,0),method="CSS")
```
```{r}
# Calculate the AICc of possible models
b <- AICc(arima(milk.log, order = c(1,1,1), seasonal = list(order = c(3,1,4), period = 12),fixed = c(0,0,NA,NA,NA,0,0,0,0), method = "CSS-ML"))
b
```
```{r}
polyroot(c(1, -0.6175))
...
```{r}
# Plot the residuals
fit1 <- arima(milk.log, order = c(1,1,1),seasonal = list(order = c(3,1,4), period = 12), method = "CSS")
res1 <- residuals(fit1)
plot.ts(res1, xlab = "")
...
shapiro.test(res1)
Box.test(res1,lag = 20, type = c("Box-Pierce"),fitdf = 2) Box.test(res1,lag = 20, type = c("Ljung-Box"),fitdf = 2) Box.test(res1ˆ2,lag = 20, type = c("Ljung-Box"),fitdf = 0) ar(res1,aic=TRUE,order.max=NULL, mehod = c("yule-walker")) # Perform test on Model 2's residual
shapiro.test(res2)
Box.test(res2,lag = 20, type = c("Box-Pierce"),fitdf = 3) Box.test(res2,lag = 20, type = c("Ljung-Box"),fitdf = 3) Box.test(res2ˆ2,lag = 20, type = c("Ljung-Box"),fitdf = 0) ar(res2,aic=TRUE,order.max=NULL, mehod = c("yule-walker")) # Compare the residual square of Model 1 and Model 2 par(mfrow=c(1,2))
acf(res1ˆ2,lag.max=40)
acf(res2ˆ2,lag.max=40)
```
